...
@Singleton
class RubricRepository @Inject constructor(private val rubricDao: RubricDao, private val clock: AppClock) {

    fun observeRubrics(): Flow<List<RubricEntity>> = rubricDao.observeActiveRubrics()

    fun observeCriteria(rubricId: Long): Flow<List<RubricCriterionEntity>> = rubricDao.observeCriteria(rubricId)

    fun observeRatingsFor(gameId: Long): Flow<List<RatingWithRubric>> = rubricDao.observeRatingsFor(gameId)
    ...
    suspend fun getCriteria(rubricId: Long): List<RubricCriterionEntity> = rubricDao.getCriteria(rubricId)

    suspend fun saveRubric(rubric: RubricEntity): Long = if (rubric.id == 0L) {
    ...
    suspend fun saveCriterion(criterion: RubricCriterionEntity): Long = if (criterion.id == 0L) {
        rubricDao.insertCriterion(criterion)
    } else {
     ...
     */
    fun computeScore(criteria: List<RubricCriterionEntity>, scores: Map<Long, Double>): Double {
     ...
     */
    suspend fun saveRating(gameId: Long, rubricId: Long, scores: Map<Long, Double>, notes: String?, existingRatingId: Long = 0): Long {
    ...
    suspend fun scoresFor(ratingId: Long): Map<Long, Double> = rubricDao.getScoresFor(ratingId).associate { it.criterionId to it.score }
    ...
    private fun criterion(rubricId: Long, name: String, description: String, weight: Double, order: Int) = RubricCriterionEntity(
        rubricId = rubricId,
        name = name,
